...
// array
const interactionTags = ['topwear', 'bottomwear', 'footwear', 'tshirt', 'shirt', 'polo', 'sweatshirt', 'hoodies', 'trousers', 'jeans', 'shorts', 'boxers', 'formals', 'trackpants', 'formal', 'sports', 'sneakers', 'socks', 'sandals', 'belted', 'red', 'green', 'yellow', 'blue', 'black', 'purple', 'orange', 'cyan', 'white', 'lilac', 'lavender', 'cotton', 'wool', 'synthetic', 'denim', 'leather', 'silk']
...
router.put("/:id", async (req, res) => {
	...
	try {
		if(type !== "purchase" && type !== "view") {
			...
			tags.forEach((tag) => {
				reqInteractions.set(tag, reqInteractions.get(tag) + (type === "purchase" ? 2 : 0.75));
			});
